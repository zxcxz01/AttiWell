...
.btn-primary.custom-btn:focus, .btn-primary.custom-btn:hover {
	color: #fff; /* 텍스트 색을 녹색으로 지정 */
	background-color: #1b7340; /* 초록색 배경색 설정 */
	border: none; /* 테두리 없애기 */
	border: 1px solid #1b7340 !important;
	/* 호버 또는 포커스 시 테두리 색을 #1b7340으로 지정 */
}
			...
			<table cellpadding="10" cellspacing="10">
				<tbody>
					...
					<button type="button"
						...
						onclick="search_goods_list('today')">오늘</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_goods_list('one_month')">최근 1개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_goods_list('two_month')">최근 2개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_goods_list('three_month')">최근 3개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_goods_list('six_month')">최근 6개월</button>
					...
					<c:otherwise>
						<c:forEach var="item" items="${newGoodsList }">
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath}/admin/goods/removeGoods.do?goods_id=${item.goods_id}">
										삭제 </td>
				...
				<tr class="no-hover">
					<td colspan=8 class="fixed"><c:forEach var="page" begin="1"
							...
							<c:if test="${section > 1 && page == 1}">
								<a
									href="${contextPath}/admin/goods/adminGoodsMain.do?section=${section-1}&pageNum=${(section-1)*10+1}">&nbsp;
									&nbsp;</a>
							</c:if>
							<a
								href="${contextPath}/admin/goods/adminGoodsMain.do?section=${section}&pageNum=${(section-1)*10+page}">${(section-1)*10+page}</a>
							<c:if test="${page == 10}">
								<a
									href="${contextPath}/admin/goods/adminGoodsMain.do?section=${section+1}&pageNum=${section*10+1}">&nbsp;
									next</a>
							</c:if>
						</c:forEach></td>
			</tbody>
