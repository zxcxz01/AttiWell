...
//fixeSearchPeriod를 form에서 받아와서 controller 전송
...
/*javascript Date() 모듈을 가져와서 현재 날짜를 end로 잡고 시작날짜를 각 조건문마다 선택한 날짜를 end에서 뺀다  */
			...
			<table cellpadding="10" cellspacing="10">
				<tbody>
					...
					<button type="button"
						...
						onclick="search_member('today')">오늘</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_member('one_month')">최근 1개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_member('two_month')">최근 2개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_member('three_month')">최근 3개월</button>
					<button type="button"
						class="btn btn-primary btn-sm custom-btn mr-3"
						onclick="search_member('six_month')">최근 6개월</button>
					...
					<tbody align=center>
						...
						<c:choose>
							<c:when test="${empty member_list}">
								<tr>
									<td colspan=5 class="fixed"><strong>조회된 회원이 없습니다.</strong>
									</td>
								</tr>
							</c:when>
							<c:otherwise>
						...
						<tr class="no-hover">
							<td colspan=8 class="fixed">
							    <c:forEach var="page" begin="1"
									...
									<a href="${contextPath}/admin/member/adminMemberMain.do?section=${section}&pageNum=${(section-1)*10+page}">${(section-1)*10+page}</a>
									<c:if test="${page == 10}">
